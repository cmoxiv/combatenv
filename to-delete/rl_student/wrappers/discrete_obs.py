...
Converts the 88-dimensional continuous observation space into a discrete
...
State Space Design (2,048 states):
    ...
    - Terrain flags: 16 combinations (building, fire, swamp, water present)

Total: 2 × 2 × 2 × 2 × 2 × 4 × 16 = 2,048 states

Usage:
    from rl_student.wrappers import MultiAgentWrapper, DiscreteObservationWrapper
...
class DiscreteObservationWrapper(gym.ObservationWrapper):
    """
    Converts continuous 88-dim observations to discrete state indices.

    This wrapper reduces the observation space from 88 continuous dimensions
    ...
    def _discretize(self, obs: np.ndarray) -> int:
        """
        Convert a single 88-dim observation to discrete state index.
    ...
    def _encode_terrain_flags(self, terrain_obs: np.ndarray) -> int:
        ...
        Flags:
            - bit 0: building present in FOV
            - bit 1: fire present in FOV
            - bit 2: swamp present in FOV
        ...
        for val in terrain_obs:
            if val < 0.01:  # EMPTY
                continue
            elif 0.2 <= val <= 0.3:  # BUILDING (0.25)
                flags |= 1  # bit 0
            elif 0.45 <= val <= 0.55:  # FIRE (0.5)
                flags |= 2  # bit 1
            elif 0.7 <= val <= 0.8:  # SWAMP (0.75)
                flags |= 4  # bit 2
    ...
    def decode_state(self, state: int) -> Dict[str, Any]:
        ...
        terrain_names = []
        if terrain_flags & 1:
            terrain_names.append("building")
        if terrain_flags & 2:
            terrain_names.append("fire")
        if terrain_flags & 4:
            terrain_names.append("swamp")
        if terrain_flags & 8:
